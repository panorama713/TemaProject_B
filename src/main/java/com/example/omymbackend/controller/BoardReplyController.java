...
//@CrossOrigin(origins="http://192.168.0.166:3000")
...
public class BoardReplyController {
    ...
    // 모든 댓글 조회
    @GetMapping("/board/detail/reply/{idx}")
    public ResponseEntity<Object> getAllBoardReply(@PathVariable("idx") Long idx) {
        logger.info("idx {}", idx);

        try {
            List<BoardReply> boardReply = boardReplyService.findReplyByIdx(idx);
    ...
    insertBoardReply(@RequestBody BoardReply boardReply) {
        ...
        try {
            List<BoardReply> savedBoardReply = boardReplyService.save(boardReply);

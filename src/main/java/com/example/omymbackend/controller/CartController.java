...
@RestController
@RequestMapping("/api")
//@CrossOrigin("http://192.168.0.166:3000/")
...
//            Vue에 에러 메세지 전송
    ...
    public ResponseEntity<Object> deleteCartByIdx(
    ...
    @DeleteMapping ("/cart/del/{userIdx}")
    public ResponseEntity<Object> deleteUserCart(
            @PathVariable("userIdx") Long userIdx){
        try{
            boolean deleteUserCart =
                    cartService.deleteUserCart(userIdx);
            if(deleteUserCart == false) {
                return new ResponseEntity<>(HttpStatus.OK);
            }else{
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            }
    ...
    public ResponseEntity<Object> insertCartItem(
        ...
        Optional<Cart> checkItem = cartService.findByItemsIdx(cart.getItemsIdx());
